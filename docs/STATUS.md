...
- **TTS replaces the browser's Web Speech API** for all pre-generatable content
  ...
  - **Azure AI Speech, `zh-HK` Neural** (v123 onward). `zh-HK-WanLungNeural`
    (male) narrates words and sentences and plays **the learner's own lines**;
    `zh-HK-HiuGaaiNeural` (female) plays the other speaker. Only three
    `zh-HK` voices exist — WanLung, HiuMaan, HiuGaai — confirmed against the live
    `voices/list` endpoint rather than the documentation. All three were
    auditioned and HiuGaai was picked as the female voice. **The two conversation
    roles were the other way round until v142** — see DES-51.
...
| DES-51 | v142 + v143 | **Conversation voices swapped — You is male, the other speaker is female.** Reported from use: the learner's own lines were `zh-HK-HiuGaaiNeural` (female) and every other character was `zh-HK-WanLungNeural` (male), which is backwards for this learner. **The casting was also contradicting the scripts.** Four conversation lines carry personal names and all four were voiced by the opposite gender: Greetings has the other speaker introduce herself as 阿梅 (aa3 mui4) "Mei" and the learner answer as "William"; Questions has the learner as "David" and the other speaker as "Sarah". Measured before changing anything — every name-bearing line in the corpus was enumerated, not sampled: those four, plus 姓Chan (sing3 Chan) in Hotels, which is a surname with no gender attached, plus two 我叫William (ngo5 giu3 William) lines that are *sentences* rather than conversation lines and so carry the narrator voice regardless. **The swap fixes all four with no content edit**, which is why no authoring pass was run. Conversation lines were also swept for gendered address terms — 先生 (sin1 saang1), 小姐 (siu2 ze2), 太太 (taai3 taai2), 女士 (neoi5 si6) — and none is aimed at either speaker; the only gendered words in the corpus are third-person family references such as 哥哥 (go1 go1) and 妹妹 (mui6 mui6), which no casting affects. **Scope is 484 files, not 1,376**: the narrator voice did not move, so words (585) and sentences (307) are untouched, and `--force=convos` already matched exactly the conversation path segment — no new flag was needed for it. **Shipped as two deploys rather than one, and the second one is the point:** the code and docs went up as v142 before the regenerated audio existed, so any device that opened the app in that window and played a conversation line cached the OLD audio under the NEW cache name — runtime caching keys on `CACHE_VERSION`, so a later push of the correct files would not have displaced it. v143 ships the audio with a fresh cache name. That is the rule this file already carried — *the version bump must land with or after the audio, never before it* — arriving as a real incident rather than a caution, and the reason it is easy to trip is that copying files into the clone and deploying them are one step in GitHub Desktop. Cost was one extra version number; unnoticed, the symptom would have been the old voices persisting on exactly the device used to check the fix. |
| DES-53 | v144 | **Two speak-back fixes from device screenshots, one invisible until you hit it and one genuinely invisible.** (a) The Speak breakdown marked a differing sentence-final particle as an error even though `fuzzyMatch()` forgives it, so a passing attempt read as a failure — 唔使喇，呢個就夠喇 (m4 sai2 laa3, ni1 go3 zau6 gau3 laa3.) came back with 啦 (laa1) for both 喇 (laa3), scored as close, and showed two red marks. The final one is now a tick. **The rule moved into `isForgivenParticleSwap()`, called by both the matcher and the panel**, since the drift existed only because one owned it. (b) 架 (gaa3) is now forgiven for 㗎 (gaa3) — 㗎 was already in `SPEAK_FINAL_PARTICLES` but its ASR spelling was not, which is why 嗰隻狗係邊個㗎 (go2 zek3 gau2 hai6 bin1 go3 gaa3?) heard as 架 (gaa3) cost an edit. **It could not simply be added to that set, and the validator is the reason it was caught rather than shipped.** `validate.js` asserts that `SPEAK_FINAL_PARTICLES` mirrors the particles actually taught in `particles.json`, in both directions — so adding 架 failed the run, correctly: the only ways to satisfy that check would have been to weaken it or to teach 架 as a vocabulary word, putting a spelling the corpus uses zero times into a lesson. Instead there is now a second, separate map, `SPEAK_PARTICLE_VARIANTS`, holding ASR spellings that are deliberately *not* taught, and the agreement check was extended to assert exactly that: a variant must be absent from `particles.json` and must map to a particle that is present. **The check did its job by refusing the first version of this fix**, which is the case for keeping agreement assertions even when they feel pedantic. **Both reported by William and his wife**, which is worth recording: two speakers producing the same substitution is the evidence class the variant-fold proposal keeps asking for. Measured while scoping: 83 of 791 spoken items contain 喇 or 啦, and in 28 of those the particles alone would consume the whole edit allowance. **Mid-sentence particles are NOT covered** — that is an open backlog question, not an oversight. **Device QA 2026-09-12: confirmed on the Pixel, the 㗎 (gaa3) case now shows a tick.** One honest limit on that confirmation, recorded so nobody over-reads it later: once the swap is forgiven the panel shows a tick and stops displaying the heard character, so a tick cannot distinguish "the recogniser returned 架 (gaa3) and we forgave it" from "the recogniser returned 㗎 (gaa3) and there was nothing to forgive". Both look identical by design. The fix is verified by the unit-level exercise of `fuzzyMatch()` and `renderSpeakBreakdown()` against both screenshots plus negative controls; the device pass confirms nothing regressed rather than proving the variant path fired. |
| DES-54 | v145 | **The checkpoint sentence-review summary now shows jyutping and English, not Chinese alone** (MOCK-31-B). Reported from use. The row was built exactly as MOCK-28 drew it, so this is a design gap rather than a regression — worth separating, because the instinct on a report like this is to look for the bug that caused it and there isn't one. Reuses the Learn screen's sentence classes rather than restyling; the only CSS change is the row aligning to the top for a three-line block. `styleguide.html` updated in the same commit, and MOCK-31 is committed alongside. **Device QA 2026-09-12: confirmed on the Pixel.** The three-line row was the open risk — eight of them turn the summary into a scroll rather than a glance — and it reads fine in use, so the fallback of dropping the English line back out (MOCK-31-A) is not being taken. |
| DES-55, DES-56 | v147 | **The tier line became a run naming every tier, and a path lesson stopped advertising a course the learner has not reached.** Reported as a navigation defect; it was not one — DES-28 and DES-29 were both working as recorded, and the tier ladder in Topics was present and functional. **The real fault was that `.tierline--split` put the tier state and the tier it offered at opposite edges of the screen**, so they read as unrelated. Three mockup passes: MOCK-32 treated it as type size and was rejected; MOCK-33 fixed the grouping; MOCK-34 stress-tested the two front runners at three tiers; MOCK-35 fixed an inversion William spotted, where the brand-coloured link outranked the tier you were actually on. **The retired `.tierline--split`, `.ladder` and `.rung` rules were deleted rather than left in place** — `dead-css` would have caught them, and did not need to. `tier-harness.js` gained coverage for both new rules, including a settable `pathCompleteCount` stub: a constant stub would have suppressed every upward row and made DES-56 pass while asserting nothing. **Device QA 2026-09-17: confirmed on the Pixel.** Both questions the build left open came back fine — the run reads as a statement rather than a chooser, so DES-28's objection did not land and the MOCK-33-E fallback is not needed; and the links still read as tappable with brand cut to the chevron, so the correction did not overshoot. |
| — | v148 | **The corpus was teaching one word two ways; standardised on 畀 (bei2) across 46 occurrences in ten files.** Reported as a recognition failure by two speakers — 俾 (bei2) coming back as 畀 (bei2) — and it was not one. 畀 is the orthodox written form and what the recogniser produces; the corpus used **both**, 畀 in 12 items and 俾 in 22, split by topic rather than by sense, so "give" was 畀 in Shopping and 俾 in Food with nothing connecting them for a learner. **The reported symptom was the cheapest part of the problem** — ten spoken items contain the word and none is short enough for one substituted character to exceed its edit allowance, so every one of them already passed while marking red. The content inconsistency was the real defect and it had gone unreported because nothing in the app puts the two spellings side by side. No audio regeneration: the pronunciation is identical and files are keyed by id, so existing audio stays correct. No code change either — standardising on the spelling the recogniser already writes removed the mismatch without a fold. `char-jyutping.json` rebuilt (644 entries, 俾 out and 畀 in, nothing else moved). CONTENT.md gains the general rule: where Cantonese admits two written forms of one word, the corpus picks one and uses it everywhere. **Device QA 2026-09-17: confirmed on the Pixel.** |
| — | v146 | **Fill-the-Gap options authored for the last nine topic conversations — coverage is now 168/168 topic user turns.** `adjectives`, `classifiers`, `clothing`, `comparisons`, `drinks`, `fruitveg`, `location`, `meatseafood`, `modals`; three user turns each, 81 strings with parallel jyutping. **Authored against the new distractor rule in CONTENT.md §3, and the rule is the substance of this change, not the count.** Every distractor is a whole grammatical turn that is wrong at the level of the conversation rather than the sentence, at least one in each turn is at least as long as the correct answer, and no turn uses more than one house fallback. **Two checks were run before the data was written, not after:** all 27 turns against rules 3 and 4 — one draft failed and was rewritten — and every Han character against the existing corpus, confirming the distractors introduce no vocabulary the app never teaches. **The older 141 turns still fail the rule** (`gap-opts-check.js` flags 120 of them) and are a separate backlog item; that is why rules 3 and 4 report rather than exit non-zero. No audio: options are tapped, never spoken. **Device QA 2026-09-12: confirmed on the Pixel** — Fill-the-Gap now appears on all nine. || DES-57 | v150 | **The speak matcher stopped grading on characters and started grading on sound, plus a CSS dead-declaration sweep.** Reported from use with two screenshots of the same sentence: 好，要粥同餃子。我好餓㗎！(hou2, jiu3 zuk1 tung4 gaau2 zi2. ngo5 hou2 ngo6 gaa3!) came back marked wrong on characters the learner had pronounced correctly. **The recogniser returns a character, not a sound**, and for a syllable with several homophones the character it picks varies between attempts on the same utterance — 粥 (zuk1) arrived as 竹 (zuk1), then 捉 (zuk1), then 祝 (zuk1) on the device QA run, and 同 (tung4) arrived as 筒 (tung4). Every substitute was a real word identical in sound and tone to the target, so the app was manufacturing false rejects out of recogniser noise and then, for an audience assumed not to read Chinese, offering as proof a character the learner could not read. `isSameSound()` resolves one reading per character and compares those; `speakCharsEqual()` wraps it and `fuzzyMatch()`, `editDistance()` and `alignChars()` all route through it, so the matcher and the panel cannot drift — DES-53's lesson applied before the drift rather than after. **Tone still counts**: 我 (ngo5) against 餓 (ngo6) stays a mismatch, confirmed on device against a real learner slip. Measured before building: of 733 corpus sentences exactly one pair shares a jyutping string, and that pair is the same sentence with different punctuation, so the rule costs essentially nothing at sentence level. **An earlier "any reading matches" draft was measured and rejected** — it folded 心 (sam1) into 新 (san1) and 係 (hai6) into 喺 (hai2), destroying distinctions the corpus teaches. Shipped alongside: 47 of 51 dead CSS declarations removed from `styles.css`, found by the new `tools/dup-css.js`, all of them already overridden so the change is invisible; the remaining 4 sit in rules shared with other selectors and need a split rather than a deletion, tracked in BACKLOG.md. **Two standing checks were found dead and repaired** — see the notes below. `resolveHeard()` now picks the closest matching candidate rather than the first, because freeing the edit budget let a truncated transcript win over an exact segment. **Twelve standing checks green** — `dup-css.js` joins the set at v150. **Device QA 2026-09-19: confirmed on the Pixel.** |
| — | v151 | **DES-57 resolved readings one character at a time; it had to read the whole string.** Reported from use one day after v150: the recogniser returned 蕃茄 (faan1 ke2) for the corpus's 番茄 (faan1 ke2) — a real variant spelling of "tomato", correctly pronounced — and it was marked wrong. **`charJyutpingSyllables()` exists to ask the dictionary for the whole string at once so it can segment words and read each character IN CONTEXT**, and the v150 `charReading()` handed it one character, discarding exactly that. In the word 蕃茄 the dictionary reads 蕃 as faan1; alone it reads faan4, a different word. So the matcher compared faan1 against faan4 and drew a cross. **The screenshot showed both halves disagreeing in one view**: the "You said" line printed faan1 under 蕃, because that line resolves the whole sentence, while the grid beside it marked the character wrong — the v140 and v144 drift shape, reached for a third time by the change meant to apply its lesson. Not a variant-spelling edge case: **355 of 7,506 corpus characters (4.7%) read differently alone than in their word**, including 呢 (ni1 / ne1), 生 (sang1 / saang1) and 坐 (co5 / zo6). Readings are now resolved per string, once, and compared by POSITION: `speakEqFor(heard, target)` returns an `(i, j, a, b)` test that `editDistance()` and `alignChars()` both take, built from the strings as they are at the moment of comparison — after normalisation and after any particle slice, since an eq built from the raw arguments would have every index shifted by whatever those steps removed. Both default to identity, so a caller that does not opt in compares exactly as it did before DES-57. **The v150 harness section passed throughout, because it tested isolated character pairs — the same wrong assumption as the code.** Rewritten to test whole sentences, with the reported case pinned, the tone pairs asserted against the predicate directly rather than through a verdict, and a guard that fails loudly if a test's position index makes its assertion vacuous. **Twelve standing checks green. Device QA 2026-09-20: confirmed on the Pixel.** |
| — | v152 | **A homophone fold was being blocked by the edit budget on short targets.** Reported the same evening as v151: 唔該晒！(m4 goi1 saai3!) heard as 唔該曬 (m4 goi1 saai3) — a true homophone, correctly said — came back "Hmm, that didn't quite match" **while the grid beneath it drew three green ticks**. `fuzzyMatch()` bailed at `if (!allowance) return false` before the comparison ran, because a three-character target gets an allowance of 0; `alignChars()` had no such gate and applied the fold. So the learner was told the attempt failed while being shown every character was right. **The fourth instance of the matcher and the panel disagreeing** (v140, v144, v151, this). The fix is one line and rests on a distinction that was implicit and should not have been: **the four-character floor exists to stop EDITS being forgiven in short strings, and a fold is not an edit.** Letting the comparison run costs no strictness — with an allowance of 0 only a distance of zero passes, so every character must either match outright or fold, and a real substitution still scores 1 and still fails: 媽媽 (maa1 maa1) heard as 嫲嫲 (maa4 maa4) is rejected exactly as before, and so is 好彩 (hou2 coi2) heard as 好菜 (hou2 coi3). **Eleven corpus conversation lines sit under the floor**, including both 唔該晒 variants, so this was reachable content rather than a theoretical edge — see the note below, because it had been dismissed as theoretical in the same session. **Twelve standing checks green. Device QA 2026-09-20: confirmed on the Pixel.** |
| DES-58, MOCK-36-pair, MOCK-36-plain, MOCK-37-deep | v153 | **The product is named Jyut, and the header, icon and status bar now come from the design system.** The name replaces the placeholder 廣東話 (gwong2 dung1 waa2) / *Cantonese Learner*: the nameplate is 粵 (jyut6) beside *Jyut* with *Learn Cantonese* beneath (`.nameplate-mark`, which retires `.header-title .zh`), the manifest `short_name` is *Jyut*, so the home-screen label is readable English, and the page title follows. The icon is 粵 (jyut6) in `--header-text` on `--header-bg`, replacing a navy-and-gold 廣 (gwong2) whose frame the Pixel's circle mask cut into four stray lines; it is exported from an outlined SVG master (`docs/design/app-icon.svg`) by a new zero-dependency page, `tools/build-icons.html`, and an `apple-touch-icon.png` is added because iOS ignores manifest icons. The status bar becomes `#481925`, a darker oxblood, because a bar matching the header merged into one block and left the nameplate reading 12px low. **Three chrome colours had drifted three ways** — `theme-color` `#1A1815`, manifest `#1a1a2e`, header oxblood — and `validate.js` now asserts the manifest against `index.html` and `--header-bg`, mutation-tested in both directions. **Invisible on screen:** nothing below the header changes. Clearance for the name is recorded in DES-58. |
| MOCK-38-corner, DES-59 | v154 | **The line can be replayed while speaking, in the checkpoint and in Chat.** Reported from the device: neither the checkpoint's *Say it in Cantonese* nor a conversation's *your turn* step could play the line, which the Say it back sheet always could. A play button now sits top-right of the speak card in a slim head row (`.speak-card-head`), disabled while the mic listens and showing the speaker icon while playing. Both screens already had a playback path, so this is a button and guards, not new audio code: the checkpoint reuses `data-sr-play` and now tracks `state.speaking`; Chat's Listen button and the new one share `playSpeakStep()`. **In the checkpoint it appears only after an attempt or a reveal** (DES-59), so the learner still has to have a go first. **Two things fixed on the way:** Chat's mic never stopped a replay in flight before opening — the other speak surfaces did — so a replay would have reached the recogniser; and a tapped oxblood checkpoint button kept an orange hover wash on Android, which the bubble play button had shown all along. **Also, the v153 icon:** the *Jyut* label reached the phone and the old icon did not, because Chrome treats a manifest icon URL as immutable. The PNGs are renamed `icon-jyut-192/512.png` so the new icon has a URL Chrome hasn't seen. Verified in a browser against the live renderers: before and after an attempt, before and after a reveal, while listening, on both kinds of conversation line. |

### Notes worth carrying forward
- **An asset that changes must change its URL — and not only for the service
  worker.** v153 replaced the icon PNGs under their old names. The page updated,
  the manifest's name reached the installed app, and the icon never did: Chrome
  on Android treats a manifest icon URL as immutable and won't re-fetch one it
  already has. The advice given at the time, that the icon would refresh within a
  day or two, was wrong for exactly this reason. v154 renamed the files. The same
  shape as the `sw.js` version bump, one layer further out, where no bump reaches.
  `tools/build-icons.html` now says so where the file names are set.
- **Four consumers of one mechanism, and one missing a guard.** Every speak
  surface calls `startSpeechRecognition()`; three stopped any audio still playing
  before opening the mic and Chat's `#mic-btn` did not. It was harmless while a
  learner's own line had no play button, and became a real bug the moment v154
  gave it one. When shared machinery has several call sites, check that each
  carries the guards the others do, rather than trusting the ones you're
  editing — the recurring form of the DES-47 lesson.
- **The styleguide's speak demos have never been styled.** Found at v154 while
  documenting `.speak-card-head`: the page has no `.speak-card`, `.mic-btn`,
  `.speak-result-*` or `.sr-*` rules, so the Say it back and checkpoint review
  demos render as plain text. A companion page can drift by never receiving a
  component's CSS, not only by holding an old copy of it. In BACKLOG.
- **A status line that contradicts its own tool's exit code.** IN_PROGRESS.md
  at v152 said *"Standing checks — twelve, all green"*. `dup-css.js` exits 1,
  and did at v152: its Check A gates on any same-selector duplicate, and four are
  known and waiting in BACKLOG as rule-splits. Found at v153 by running all twelve
  rather than trusting the line; the count is 4 before and after v153, so nothing
  regressed. **"Green" in a status line should mean exit 0**, and a check that is
  red by design until a known item lands should be recorded as *red, N known*,
  not rounded up. Otherwise the fifth duplicate arrives looking identical to the
  four already accepted.
- **Hand-copied values outside the stylesheet drift the same way the particle
  mirror did.** The status bar colour lived in `index.html` (`#1A1815`) and in
  `manifest.json` (`#1a1a2e`), neither matched the other or the oxblood header,
  and no check could see any of it because none of the three is a CSS variable.
  Same class as `SPEAK_FINAL_PARTICLES` (v129) and the same fix: read the literal
  out of each file and assert agreement. It is one more entry for the
  agreement-pairs item in BACKLOG.
- **An SVG comment may not contain a double hyphen.** The icon master's first
  draft named `--header-bg` inside its XML comment. XML forbids `--` in a comment,
  so the whole file failed to parse, and a browser reports that only as an image
  that won't load, with no error anywhere. Any SVG or XML file that describes CSS
  custom properties in a comment has this trap; write *the header-bg token*
  instead.
- **"That path is unreachable" is a measurement, not an intuition.** While
  scoping DES-57 the short-target branch of `fuzzyMatch()` was dismissed as
  theoretical, on the reasoning that every speak surface takes a sentence and
  sentences are longer than four characters. The first half was right and the
  second was never checked: **11 corpus conversation lines are under four Han
  characters**, including 唔該晒 (m4 goi1 saai3), 好彩 (hou2 coi2) and 多謝你
  (do1 ze6 nei5) — short, extremely common, and exactly the kind of line a
  learner repeats most. One of them was reported from use within a day. The cost
  was not just the defect but that a *decision* was taken on the strength of the
  assumption: the question of whether folding should apply to short targets was
  closed without being answered. **Where a branch is about to be ruled out, count
  the content that reaches it** — the count took under a minute and would have
  changed the answer.
- **A test built from the same wrong assumption as the code will always agree
  with it.** The DES-57 harness section at v150 asserted on isolated character
  pairs — `isSameSound('粥','竹')` — and passed. The bug it was written to
  prevent was that the code resolved readings per character instead of per
  string, so the test and the defect shared one premise and the test could only
  ever confirm it. Shipped green, reported from use the next day. **The guard is
  to write the test at the level the user experiences**, which for speech is a
  whole sentence, not a character pair — and where a pass could come from either
  the rule or some other mechanism, assert the rule directly. A second version of
  the same block then reported green on four-character targets because the edit
  allowance of 1 was absorbing the substitution, measuring the budget and calling
  it the rule; and a third had an off-by-one that compared 喇 with 喇. **Three
  faults in one test section is the finding, not three fixes** — the section now
  carries an explicit vacuousness guard for exactly that reason.
- **A standing check that fails to LOAD reports nothing, and from outside looks
  exactly like one nobody ran.** `asr-replay-harness.js` and
  `sentence-review-harness.js` both lift functions out of `app.js` by name into a
  `vm` sandbox. When `fuzzyMatch()` gained a call to `isForgivenParticleSwap()` at
  v144, neither harness lifted the new function, so both threw a `ReferenceError`
  on startup and **were dark for every release from v144 to v148** — including
  v148, whose whole subject was speech matching. Nothing noticed because nothing
  reads their exit codes; the checks are run by hand and a stack trace scrolls
  past like any other output. Found at v150 only because the same functions were
  being changed again. Two guards worth having: a runner that aggregates exit
  codes across all eleven checks, so a dead one is visible as a failure rather
  than as silence (BACKLOG.md), and the habit of reading a check's **pass lines**
  rather than its absence of failures. The lift-by-name pattern is the underlying
  fragility — it is what makes these harnesses able to test the shipped code
  rather than a copy of it, and the cost is that every new call inside a lifted
  function is a silent break waiting to happen.
- **Loosening one rule frees budget another rule was quietly consuming.**
  DES-57 made homophones cost zero edits. That was correct in itself, and it
  immediately broke something unrelated: `resolveHeard()` returned the first
  candidate that matched, and with the allowance no longer spent on a homophone
  substitution, a **truncated** accumulation came in under budget and won over a
  segment that held the full correct utterance. The learner would have been shown
  一家二係 (jat1 gaa1 ji6 hai6) in place of what they actually said, 一加二係三
  (jat1 gaa1 ji6 hai6 saam1). Both verdicts were "pass", so no test asserting pass
  rates would have seen it — `asr-replay-harness.js` caught it only because it
  asserts *which string is shown*, not just whether the attempt matched. **When a
  leniency rule is widened, the thing to re-examine is every other rule that was
  sized against the same budget**, not just the pass rate.
- **Check whether the corpus disagrees with itself before making the app
  tolerant of disagreement.** Two sightings were logged on the variant-fold watch
  list before anyone checked the obvious thing, and neither belonged there: 架
  (gaa3) was already covered by the particle set, and 畀 (bei2) was a corpus that
  spelled one word two ways (v148). **A fold would have "fixed" both** — the
  speak-back marks would have gone green and the underlying faults would have
  stayed, one of them a content inconsistency a learner meets directly. The shape
  to watch for: a tolerance mechanism is attractive precisely because it makes
  the symptom disappear without anyone having to find the cause, and the symptom
  disappearing is then read as evidence the cause was understood. Ask what the
  data says before asking what the matcher should forgive.
- **A rule that decides and a panel that reports must read from one place.**
  The sentence-final particle rule lived inside `fuzzyMatch()` and nowhere else,
  so `alignChars()` — which draws what the learner actually sees — had no idea a
  character had been forgiven and marked it red (DES-53, v144). Nothing was
  broken in the usual sense: the matcher was right, the alignment was right, and
  every standing check passed. **The defect existed only in the gap between
  them**, and it is the same gap DES-41 closed on the colour of a near-miss and
  the v140 play button closed on an overridden declaration. The shape: when a
  decision and its presentation are computed separately from the same inputs,
  they agree until one of them gains a special case. Look for it wherever a
  feature has a "does it pass" function and a "show them why" function.
- **A default carried across a provider migration is a decision nobody made.**
  The conversation casting shipped at v123 with the comment "the defaults below
  mirror the Google casting they replaced" — and mirroring is exactly what
  happened: the roles were ported rather than re-judged, and survived nineteen
  versions with the learner's own lines in the wrong voice and four scripted
  names contradicting the voice saying them (DES-51, v142). **The migration
  checked what it was worried about and nothing else.** v122's investigation was
  entirely phonetic — could the new provider say a bare syllabic nasal — so voice
  *quality* was tested exhaustively and voice *assignment* rode along untested.
  The shape to watch for: when a component is replaced, its configuration gets
  re-pointed rather than re-decided, and the parts that were never the reason for
  the migration are the parts that go unexamined. Any config comment that says it
  mirrors what came before is worth one deliberate pass.
...
- **The twelve standing checks** (IN_PROGRESS.md; eleven exit 0, and `dup-css.js`
  exits 1 on four known duplicates), which cover what is mechanically
